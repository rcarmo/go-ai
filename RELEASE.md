...
- Candidate `fcd8270faee46a2eead7ef13e054f96704791ff4` and CI run `33890620670` are superseded/rejected. Corrective local gates passed: `make check`, shuffle, race, vet, staticcheck, logging, repro, SBOM, vuln, and license. Corrective clean-checkout validation passed with `go test ./...`, committed inventory validation/self-test, catalog delta validation/self-test, 142-row manifest validation, model/image regeneration comparators, and `git diff --check`. SHA-linked SBOM/hosted CI will be rerun after the corrective commit.

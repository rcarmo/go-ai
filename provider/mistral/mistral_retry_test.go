...
func TestStreamMistralRetries429AndSucceeds(t *testing.T) {
	attempts := 0
	...
	model := &goai.Model{ID: "mistral-large", Provider: goai.ProviderMistral, Api: goai.ApiMistralConversations, BaseURL: server.URL}
	...
	for ev := range streamMistral(context.Background(), model, convCtx, opts) {
		switch e := ev.(type) {
		case *goai.DoneEvent:
			done = e

#!/usr/bin/env python3
"""Validate exact catalog deltas for the v0.85.1 -> v0.87.0 audit."""
...
EXPECTED_FILES = {
    ...
    "images-v0851.jsonl": (52, "ee84a20ae8d536f90a223fd99fba0111a2b8bcc0a742682ff8363d987ef71011"),
    "images-v0870.jsonl": (54, "1a203c24d75d77c016a7497109dd6f2c8d390b29a7a3a08de60589b0c3c42f03"),
}
EXPECTED_DELTAS = {"text": (149, 58, 986), "images": (2, 0, 4)}
EXPECTED_CURRENT = {"text": (1445, 41, 10), "images": (54, 1, 1)}
...
def canonical(record: dict, *, ignore_input_limits: bool = False) -> str:
    copy = dict(record)
    copy.pop("_key", None)
    if ignore_input_limits:
        copy.pop("inputLimits", None)
...
def fail(message: str) -> int:
    print(f"v0.87.0 catalog delta validation failed: {message}", file=sys.stderr)
    return 1
...
def validate(root: pathlib.Path = ROOT) -> int:
    ...
    try:
        text_old = load(docs / "text-v0851.jsonl")
        text_new = load(docs / "text-v0870.jsonl")
        images_old = load(docs / "images-v0851.jsonl")
        images_new = load(docs / "images-v0870.jsonl")
    except Exception as exc:  # noqa: BLE001 - CLI diagnostic
        return fail(str(exc))
    for label, old, new, ignore_limits in [("text", text_old, text_new, True), ("images", images_old, images_new, False)]:
        added = set(new) - set(old)
        removed = set(old) - set(new)
        changed = {key for key in set(old) & set(new) if canonical(old[key], ignore_input_limits=ignore_limits) != canonical(new[key], ignore_input_limits=ignore_limits)}
    ...
    print("v0.87.0 catalog delta validation passed")
    return 0
...
def self_test() -> int:
    with tempfile.TemporaryDirectory(prefix="go-ai-v0870-catalog-delta-test-") as td:
        ...
        for name in EXPECTED_FILES:
            ...
            mutate_first_metadata(copy / "docs" / "v0870" / name)
            proc = subprocess.run([sys.executable, str(copy / "scripts" / "validate-v0870-catalog-delta.py")], cwd=copy, text=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
            if proc.returncode == 0:
                return fail(f"corrupted {name} unexpectedly validated")
    print("v0.87.0 catalog delta negative self-test passed")
    return 0

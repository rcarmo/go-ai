...
func TestV08010KimiCodingAdaptiveThinkingCompat(t *testing.T) {
	goai.RegisterBuiltinModels()
	for _, id := range []string{"kimi-for-coding", "k3", "k2p7", "kimi-for-coding-highspeed", "kimi-k2-thinking"} {
	...
	for _, level := range []goai.ModelThinkingLevel{goai.ModelThinkingLevel(goai.ThinkingOff), goai.ModelThinkingLevel(goai.ThinkingMinimal), goai.ModelThinkingLevel(goai.ThinkingLow), goai.ModelThinkingLevel(goai.ThinkingMedium), goai.ModelThinkingLevel(goai.ThinkingHigh), goai.ModelThinkingLevel(goai.ThinkingXHigh)} {
...
func TestV08010XAIRemovedModelsAndOpenCodeGoAdditions(t *testing.T) {
	goai.RegisterBuiltinModels()
	...
	if grok45 == nil || !grok45.Reasoning || grok45.ContextWindow != 500000 || grok45.MaxTokens != 500000 || grok45.Cost.Input != 2 || grok45.Cost.Output != 6 || grok45.Cost.CacheRead != 0.5 {
		t.Fatalf("expected opencode-go/grok-4.5 v0.80.10 metadata, got %#v", grok45)
	}

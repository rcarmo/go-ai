# v0.85.1 whole-corpus test disposition manifest

Audit target: official upstream `github.com/earendil-works/pi` / `@earendil-works/pi-ai` tag `v0.85.1`, SHA `d981de1229ef899957bbe968bc8dcda02a21f477`.

Source corpus is unchanged from v0.85.0 and committed at `docs/v0851/test-corpus-142.txt` (142 rows, SHA-256 `56f8742065a4ad01d73e5aee53035324f2e7333a735222ab15db870819e29065`).

## Summary

- Upstream test files: **142**
- DETERMINISTIC-PORTED / adapted deterministic coverage: **123**
- N/A live/credential/JS-runtime/generator-policy/Workers-binding adapted-only: **19**
- Unclassified upstream test files: **0**
- Achieved (`DETERMINISTIC + N/A`): **142 / 142**

Changed upstream test files in v0.85.1: **3** (`cache-retention.test.ts`, `max-thinking.test.ts`, `supports-xhigh.test.ts`).
...
| 3 | `test/anthropic-auth-token.test.ts` | DETERMINISTIC-PORTED | Carried forward; not changed in v0.85.1. Existing auth-token/Copilot/OAuth header tests remain; `v0850_beta_override_test.go` adds model-level `Anthropic-Beta` override parity so configured beta headers replace generated betas. |
| 4 | `test/anthropic-cache-write-1h-cost.test.ts` | DETERMINISTIC-PORTED | Carried forward; not changed in v0.85.1. Existing `anthropic_cache_write_1h_cost_test.go` continues to cover 1h cache-write usage/cost and fallback zero/default handling. |
...
| 13 | `test/anthropic-sse-parsing.test.ts` | DETERMINISTIC-PORTED | Carried forward; not changed in v0.85.1. Existing Anthropic SSE parsing tests plus `v0850_mid_conversation_effort_test.go` cover signed thinking/reasoning deltas, terminal pending errors, managed `providerThinkingLevel`, and raw stop behavior. |
...
| 18 | `test/assistant-message-frame.test.ts` | DETERMINISTIC-PORTED | Carried forward; not changed in v0.85.1. `assistant_message_frame.go` plus `tests/assistant_message_frame_v0850_test.go` cover compact stream frame encode/reduce for text, thinking/signatures/redaction, tool calls/JSON deltas, namespaces, start ordering, and terminal omission. |
...
| 22 | `test/baseten-models.test.ts` | DETERMINISTIC-PORTED | Carried forward; not changed in v0.85.1. `tests/models_v0850_catalog_test.go` verifies Baseten Kimi K2.7 Code image/text metadata against regenerated exact catalog. |
...
| 33 | `test/cache-retention.test.ts` | DETERMINISTIC-PORTED | v0.85.1 changed. `inference/provider/openairesponses/v0851_cache_retention_test.go` covers production Responses request serialization for explicit prompt-cache models: `none` -> `prompt_cache_options:{mode:"explicit"}`, `long` -> `prompt_cache_options:{ttl:"30m"}`, short/default omission, long-without-support omission, and legacy compatible `prompt_cache_retention:"24h"`. |
...
| 37 | `test/constrained-sampling.test.ts` | DETERMINISTIC-PORTED | Carried forward; not changed in v0.85.1. Existing strict/constrained sampling tests remain green; generator/catalog changes are covered by exact regeneration and provider schema tests. |
...
| 48 | `test/generate-models-strict.test.ts` | N/A/adapted-generator-policy | Carried forward; not changed in v0.85.1. Private TS generator strict helper behavior is represented by exact Go generator parsing/emission plus clean comparator/fault gates, not a helper-for-helper port. |
| 49 | `test/github-copilot-anthropic.test.ts` | DETERMINISTIC-PORTED | Carried forward; not changed in v0.85.1. Existing GitHub Copilot Anthropic tests cover Claude Code headers/tool-name normalization and no incompatible Anthropic beta emission; managed-effort beta generation is separately covered for Anthropic models. |
| 50 | `test/github-copilot-oauth.test.ts` | DETERMINISTIC-PORTED/ADAPTED | Carried forward; not changed in v0.85.1. Existing OAuth policy/runtime tests remain; JS credential-store UI aspects are N/A, while token/header/catalog behavior is deterministic in Go. |
...
| 69 | `test/max-thinking.test.ts` | DETERMINISTIC-PORTED | v0.85.1 changed. `tests/models_v0851_catalog_test.go` and `tests/supports_xhigh_upstream_test.go` cover GPT-6 Astra direct OpenAI/Azure/Codex thinking maps including xhigh/max and Codex minimal->low behavior. |
...
| 77 | `test/node-http-proxy.test.ts` | DETERMINISTIC-PORTED | Carried forward; not changed in v0.85.1. `retry.go` implements v0.85.0 NO_PROXY/no_proxy matching including root/subdomain and bracketed IPv6; `tests/v0850_uuid_proxy_test.go` covers deterministic proxy bypass behavior. |
...
| 82 | `test/openai-codex-stream.test.ts` | DETERMINISTIC-PORTED | Carried forward; not changed in v0.85.1. `inference/provider/openaicodex/v0850_terminal_sse_test.go` proves terminal SSE frames are processed without a trailing blank line; existing Codex SSE/WS tests remain green. |
| 83 | `test/openai-completions-cache-control-format.test.ts` | DETERMINISTIC-PORTED | Carried forward; not changed in v0.85.1. Existing OpenAI cache-control format tests remain green against regenerated compat metadata. |
...
| 90 | `test/openai-completions-thinking-as-text.test.ts` | DETERMINISTIC-PORTED | Carried forward; not changed in v0.85.1. Existing OpenAI thinking-as-text tests remain green against regenerated model compat. |
...
| 92 | `test/openai-completions-tool-choice.test.ts` | DETERMINISTIC-PORTED | Carried forward; not changed in v0.85.1. Existing OpenAI tool-choice request-shape tests remain green. |
| 93 | `test/openai-completions-tool-result-images.test.ts` | DETERMINISTIC-PORTED | Carried forward; not changed in v0.85.1. Existing OpenAI tool-result image routing/downgrade tests remain green. |
| 94 | `test/openai-completions-vllm-priority.test.ts` | DETERMINISTIC-PORTED | Carried forward; not changed in v0.85.1. `inference/provider/openai/v0850_vllm_priority_test.go`, `compat.go`, `openai.go`, and regenerated catalog metadata prove `completionsCompat.vllmPriority` serializes as OpenAI `priority` and is omitted when unset. |
...
| 96 | `test/openai-responses-compat.test.ts` | DETERMINISTIC-PORTED | Carried forward; not changed in v0.85.1. `inference/provider/openairesponses/v0850_max_output_tokens_test.go` covers `responsesCompat.supportsMaxOutputTokens` default/false request serialization. |
...
| 100 | `test/openai-responses-namespace.test.ts` | DETERMINISTIC-PORTED | Carried forward; not changed in v0.85.1. Existing namespace/additional-tools Responses tests remain green; regenerated catalog/provider metadata retained. |
...
| 105 | `test/openrouter-cache-control-models.test.ts` | DETERMINISTIC-PORTED | Carried forward; not changed in v0.85.1. Exact catalog regeneration plus OpenRouter cache-control model tests cover changed model metadata. |
...
| 111 | `test/pi-messages.test.ts` | DETERMINISTIC-PORTED | Carried forward; not changed in v0.85.1. `inference/provider/pimessages/pimessages_test.go` covers `providerThinkingLevel` propagation from terminal server events into the assistant message. |
| 112 | `test/pre-generation-error.test.ts` | DETERMINISTIC-PORTED/ADAPTED | Carried forward; not changed in v0.85.1. Upstream asserts JavaScript `streamSimple` throws synchronously before creating an `AssistantMessageEventStream` when auth is missing. Go provider APIs expose `<-chan Event`; equivalent production-path behavior is deterministic `ErrorEvent` before dispatch with exact missing-key messages across direct providers, covered by provider auth tests and documented as channel-adapted rather than a TS construction throw. |
...
| 117 | `test/qwen-token-plan-models.test.ts` | DETERMINISTIC-PORTED | Carried forward; not changed in v0.85.1. `tests/qwen_token_plan_upstream_test.go` and `tests/models_v0850_catalog_test.go` cover updated Qwen Token Plan model allowlists/metadata. |
...
| 124 | `test/supports-xhigh.test.ts` | DETERMINISTIC-PORTED | v0.85.1 changed. `tests/supports_xhigh_upstream_test.go` adds GPT-6 Astra support-level assertions across OpenAI, Azure, OpenAI Codex, and OpenRouter generated metadata. |
...
| 129 | `test/tool-call-id-normalization.test.ts` | DETERMINISTIC-PORTED | Carried forward; not changed in v0.85.1. Existing tool-call ID normalization tests remain green across Anthropic/OpenAI/Codex paths. |
...
| 134 | `test/uuid.test.ts` | DETERMINISTIC-PORTED | Carried forward; not changed in v0.85.1. `utils_text_uuid.go` accepts optional timestamp millis while preserving monotonic RFC9562 layout; `tests/v0850_uuid_proxy_test.go` covers deterministic timestamp/version/variant behavior. |
...
| 137 | `test/xai-responses.test.ts` | DETERMINISTIC-PORTED | Carried forward; not changed in v0.85.1. Existing xAI Responses tests plus regenerated catalog verify Grok routing/request shape and removal of stale Grok 4 entries. |

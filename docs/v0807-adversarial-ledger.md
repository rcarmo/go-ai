...
| `api: "pi-messages"` added and exported in TS. | **PORTED.** | Added exported `ApiPiMessages` constant and inference alias, plus a public Go `inference/provider/pimessages` runtime for custom model metadata using the Radius/pi-messages backend. |
| `src/api/pi-messages.ts` and `test/pi-messages.test.ts`. | **PORTED.** | Implemented POST `<baseUrl>/messages` with Radius/API-key env resolution, debug query support, custom headers/hooks, SSE conversion for text/thinking/tool/error/done events, rewrite and response-failure diagnostics, missing-key and missing-terminal errors, and deterministic HTTP/SSE tests. |
| OAuth Radius helper (`utils/oauth/radius.ts`) and `RADIUS_API_KEY`. | **PORTED.** | Added `oauth.RadiusProvider` registered as `radius`, with `/v1/oauth` discovery, device-code OAuth token exchange/refresh, typed OAuth response errors, `/v1/config` gateway catalog caching with previous-config fallback, and `pi-messages` model injection. Deterministic local-server tests cover discovery/device authorization, full device-code login polling, pending→success, `slow_down`, `expired_token`, `access_denied`, context cancellation/deadline at the internal context-aware poll boundary, refresh/config loading, cached fallback, and typed errors. Public `Login` still uses `context.Background()` because the existing Go OAuth interface has no context parameter; cancellation is therefore verified below that API boundary. |
...
| Upstream tests: renamed OpenAI Responses compat test, added `pi-messages.test.ts`, minor Anthropic e2e prompt edits. | **PORTED where deterministic.** | Rename/prompt edits do not change deterministic Go behavior. New pi-messages runtime tests are ported under `inference/provider/pimessages`; Radius OAuth behavior is covered under `oauth/radius_test.go`. |

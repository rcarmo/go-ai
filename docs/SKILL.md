...
- Read the current upstream package from the installed path:
  - `/usr/local/lib/bun/install/global/node_modules/@earendil-works/pi-ai`
...
```bash
cat /usr/local/lib/bun/install/global/node_modules/@earendil-works/pi-ai/package.json | jq -r .version
```

...
- Provider catalog parity is checked by regenerating `models_generated.go` from upstream split `providers/*.models.ts`; `scripts/compare-upstream-models.py` reads exact upstream provider maps directly and currently verifies 1057 unique text-model provider/id pairs across 35 providers from upstream `0e6909f050eeb15e8f6c05185511f3788357ddb3`.

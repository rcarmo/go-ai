...
| `TestClampMaxTokensToContextUsesEstimateAndSafetyWindow` | `simple_options_clamp_test.go` | context-aware max-token clamping | Guards upstream `clampMaxTokensToContext` safety-window behavior. |
| `TestClampMaxTokensToContextHonorsMinimumAndUnboundedModels` | `simple_options_clamp_test.go` | context-aware max-token clamping | Guards upstream `clampMaxTokensToContext` min-token and unbounded-model behavior. |

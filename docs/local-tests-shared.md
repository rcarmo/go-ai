...
- Upstream `clampMaxTokensToContext` is covered by `simple_options_clamp_test.go` using canonical upstream literals from `simple-options.ts`: `CONTEXT_SAFETY_TOKENS = 4096`, `MIN_MAX_TOKENS = 1`; boundary case `contextWindow=5000`, `"hello"` estimate `2`, requested `2000` clamps to `902`. Canonical request-param clamp scope is Anthropic Messages and Bedrock Converse Stream only. OpenAI Completions/Responses/Codex, Azure Responses, Google/Vertex, Gemini CLI, and Mistral intentionally pass raw `options.maxTokens` like upstream; raw `2000` boundary tests guard that non-clamping behavior.
...
| `TestBuildGoogleRequestUsesRawMaxOutputTokensWithoutContextClamp` | `inference/provider/google/google_raw_max_tokens_upstream_test.go` | max-token request parity | Guards upstream raw `options.maxTokens` behavior for Google/Vertex (no context clamp). |
...
| `TestBuildMistralRequestUsesRawMaxTokensWithoutContextClamp` | `inference/provider/mistral/mistral_raw_max_tokens_upstream_test.go` | max-token request parity | Guards upstream raw `options.maxTokens` behavior for Mistral (no context clamp). |
...
| `TestBuildRequestBodyUsesRawMaxTokensWithoutContextClamp` | `inference/provider/openai/openai_raw_max_tokens_upstream_test.go` | max-token request parity | Guards upstream raw `options.maxTokens` behavior for OpenAI Completions (no context clamp). |
...
| `TestBuildCodexRequestUsesRawMaxOutputTokensWithoutContextClamp` | `inference/provider/openaicodex/codex_raw_max_tokens_upstream_test.go` | max-token request parity | Guards upstream raw `options.maxTokens` behavior for OpenAI Codex (no context clamp). |
...
| `TestBuildResponsesRequestUsesRawMaxOutputTokensWithoutContextClamp` | `inference/provider/openairesponses/responses_raw_max_tokens_upstream_test.go` | max-token request parity | Guards upstream raw `options.maxTokens` behavior for OpenAI/Azure Responses (no context clamp). |

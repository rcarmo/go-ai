...
- Upstream `clampMaxTokensToContext` is covered by `simple_options_clamp_test.go` using canonical upstream literals from `simple-options.ts`: `CONTEXT_SAFETY_TOKENS = 4096`, `MIN_MAX_TOKENS = 1`; boundary case `contextWindow=5000`, `"hello"` estimate `2`, requested `2000` clamps to `902`. The clamp is wired into provider request builders with boundary tests for OpenAI Completions, OpenAI Responses/Codex, Anthropic, Bedrock, Google, and Mistral.

...
- Local Go test functions inventoried: **464**
...
- Provider catalog parity is checked by regenerating `models_generated.go` from upstream split `providers/*.models.ts`; `scripts/compare-upstream-models.py` reads exact upstream provider maps directly and currently verifies 1065 generated text models across 35 providers from published upstream v0.80.7 (`818d67457cdd6b60bce6b121d16b23141c252dd8`).
...
- v0.80.7 Radius OAuth parity is covered by `oauth/radius_test.go`: local-server tests exercise `/v1/oauth` discovery, device authorization, token refresh, `/v1/config` catalog caching/model injection, previous-config fallback, and typed OAuth errors.

...
Adoption: regenerated the text catalog from exact v0.82.1 model data (`1109 models / 37 providers`); image catalog remains exact at `40 image models / 1 provider`; added `ModelsError` cause preservation; added ETag/Last-Modified model-catalog revalidation helpers, store preservation, and Radius 304 cache reuse; ported Claude Opus 5/Bedrock metadata; routed Radius OAuth refresh/device flows directly through gateway endpoints with legacy-discovery fallback; added Anthropic `ANTHROPIC_AUTH_TOKEN` bearer-header env auth.
...
Adoption: regenerated the text catalog from exact v0.82.0 grouped JSON-backed model data (`1116 models / 37 providers`) and the image catalog from exact v0.82.0 image metadata (`40 image models / 1 provider`); extended generator/comparator support for v0.82 API-grouped JSON shards; ported constrained sampling metadata/conversion across OpenAI Completions, Responses/Azure-shared, and Codex Responses; ported OpenRouter OAuth and Kimi Code subscription OAuth; production-wired abortable provider/DNS retry through actual OpenAI, Responses/Azure-shared, Anthropic, and Codex HTTP paths; updated deterministic regressions for Together reasoning maps, OpenRouter Kimi/GLM pricing, xAI/OpenCode/OpenRouter/Vercel metadata, and image model counts.
...
Mechanical diff found 25 `packages/ai` paths changed. Go-facing adoption: regenerated the text provider catalog to exact v0.80.7 package output (`1065 models / 35 providers`), added the exported `ApiPiMessages` constant/alias for custom model metadata round-trip, ported the public `pi-messages` Radius/custom backend (`POST /messages`, debug query, headers/hooks, SSE event conversion, diagnostics, missing-key/missing-terminal errors, cancellation cleanup), ported the Radius OAuth helper into the public Go `oauth` package with discovery/device flow/refresh/config-cache model integration, and updated deterministic catalog regressions for v0.80.7 pricing/window/token changes plus split GPT-5.6 IDs. Image model catalog did not change.
